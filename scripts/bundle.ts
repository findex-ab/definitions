  ...
  const generateExports = () => {
    ...
    const generate = (basedir?: string) => {
      ...
      iterFiles(tmpDir, (file: fs.Dirent) => {
        ...
        let importpath = ((basedir ? ('./' + path.join(basedir, filepath.replace(process.cwd(), '')).replace('/_tmp_', '').slice(1)) : filepath.replace("src/", ".")).replace(".ts", "")).replace('tmp_/', '');

        //if (basedir === './') importpath = importpath.replace('/', '');
    ...
    //fs.writeFileSync(path.join(tmpDir, '/index.ts'), generate('./'), { encoding: 'utf8' });
    fs.writeFileSync(path.join(tmpDir, 'index.ts'), generate('./'), { encoding: 'utf8' });
  };
